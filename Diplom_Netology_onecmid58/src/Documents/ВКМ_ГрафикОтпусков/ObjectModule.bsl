	...
	Для Каждого Строка Из ОтпускаСотрудников Цикл
		...
		Если ЗначениеЗаполнено(Строка.ВКМ_ДатаНачала) И ЗначениеЗаполнено(Строка.ВКМ_ДатаОкончания) Тогда
			Если Строка.ВКМ_ДатаНачала > Строка.ВКМ_ДатаОкончания Тогда
				ОбщегоНазначения.СообщитьПользователю(СтрШаблон("В строке №%1 не верно заполнен период",
					Строка.НомерСтроки));
				Отказ = Истина;
			КонецЕсли;
...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Пока Результат.Следующий() Цикл
		...
		Если ПревышеноДней > 0 Тогда
			...
			ОбщегоНазначения.СообщитьПользователю(СтрШаблон("Привышено количество дней отпуска в год по сотруднику %1",
				Результат.Сотрудник));
